...
- Cron endpoints live under `functions/api/cron/` and are HTTP-triggered by an external scheduler — every cron handler verifies `Authorization: Bearer ${CRON_SECRET}` and 401s on mismatch. Current crons: `welcome-emails`, `abandoned-checkouts`, `scheduled-vouchers`, `refresh-google-reviews`, `post-flight-review-sms`, `lead-magnet-emails`, `scrape-cold-leads`.
